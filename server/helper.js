...
//token isn't required? Send just the dbResp.      : Otherwise, send the dbResp and a new token pair
    ...
    !reqToken ? res.status(200).sendStatus(dbResp).end() : res.status(200).sendStatus({userData, auth: {id_token: createIdToken(userData), access_token: createAccessToken()}});
  } else {
...
module.exports.respErr = (dbResp, req, res) => {
  ...
  res.status(500).sendStatus({error: dbResp}).end();
};
